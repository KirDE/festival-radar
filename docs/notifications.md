...
Production systemd owns dispatch scheduling. `festival-radar-notifications.timer` invokes the loopback-only dispatcher every ten minutes with a non-overlapping file lock. The GitHub notification workflow is a manual diagnostic and does not own the schedule. Immediate notifications are due at creation, daily digests at the next 08:00 UTC, and weekly digests at the next Monday 08:00 UTC.
...
`/api/health/notification-scheduler` separates scheduler freshness from delivery readiness. It reports `schedulerStatus`, redacted provider booleans, `deliveryReady`, the current run counts, and the most recent non-empty delivery outcome. A fresh empty dispatcher run cannot erase the last delivery evidence, and a healthy timer with no configured provider is `degraded`/HTTP 503 rather than an overall healthy notification feature. Per-delivery attempts, timestamps and bounded provider status errors are retained in PostgreSQL. Provider request bodies contain only the configured destination and notification content.

## Recovery

1. Inspect the systemd service result and `/api/health/notification-scheduler`, then inspect delivery status in PostgreSQL by ID. Never paste endpoint or secret columns into an issue.
2. Correct provider configuration or availability, redeploy the protected environment when configuration changed, and start `festival-radar-notifications.service`. Pending deliveries keep their scheduled retry time; stale claims recover after 15 minutes.
