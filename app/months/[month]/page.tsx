...
export async function generateStaticParams(){const {festivals}=await getCatalog();return [...new Set(festivals.map(festivalMonth).filter((value):value is string=>Boolean(value)))].map((month)=>({month}));}
