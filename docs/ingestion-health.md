...
The per-run `summary.json` is also the adapter coverage artifact. Every source reports its configured `extractionPath`, any manual-review reason, extracted evidence fields, and the persisted `lastSuccessfulExtraction`. A successful HTTP check without supported evidence does not advance that extraction timestamp.
