...
export type SourceHealth = { consecutiveFailures: number; lastAttemptAt: string; lastSuccessfulCheck?: string; lastSuccessfulExtraction?: string; lastFailure?: { at: string; error: string; httpStatus?: number } };
...
export async function runIngestion(input: { sources: FestivalSource[]; festivals: Festival[]; outputDirectory: string; stateFile: string; failureThreshold: number; now?: () => Date; fetchOptions?: FetchOptions }) {
  ...
  for (const source of input.sources) {
    ...
    try {
      ...
      const lastSuccessfulExtraction = candidate.evidence.length ? fetchedAt : previous?.lastSuccessfulExtraction;
      state.sources[source.festivalSlug] = { consecutiveFailures: 0, lastAttemptAt: fetchedAt, lastSuccessfulCheck: fetchedAt, lastSuccessfulExtraction };
      summary.processed += 1;
      ...
      summary.results.push({ festivalSlug: source.festivalSlug, status, attempts, extractionPath: source.strategies, manualReviewReason: source.manualReviewReason ?? null, evidenceFields: candidate.evidence.map(({ field }) => field), lastSuccessfulExtraction: lastSuccessfulExtraction ?? null, changes: result.changes.length, reviewReasons: result.reviewReasons });
    } catch (error) {
      const previous = state.sources[source.festivalSlug];
      ...
      state.sources[source.festivalSlug] = { consecutiveFailures, lastAttemptAt: fetchedAt, lastSuccessfulCheck: previous?.lastSuccessfulCheck, lastSuccessfulExtraction: previous?.lastSuccessfulExtraction, lastFailure: { at: fetchedAt, error: message, httpStatus } };
      summary.fetchErrors += 1;
      if (consecutiveFailures >= input.failureThreshold) summary.escalatedFailures += 1;
      summary.results.push({ festivalSlug: source.festivalSlug, status: consecutiveFailures >= input.failureThreshold ? "escalated_failure" : "fetch_error", extractionPath: source.strategies, manualReviewReason: source.manualReviewReason ?? null, error: message, httpStatus, attempts, consecutiveFailures, lastSuccessfulCheck: previous?.lastSuccessfulCheck ?? null, lastSuccessfulExtraction: previous?.lastSuccessfulExtraction ?? null });
    }
  }
